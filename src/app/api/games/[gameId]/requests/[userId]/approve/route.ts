...
export async function PATCH(
    ...
    try {
        ...
        // Remove the user from joinRequests and add to currentPlayers
        ...
        await Notification.create({
            ...
            game: gameId,
            message: `Has approved your request to join "${game.title}"`,
            isRead: false,
            type: NotificationType.REQUEST_APPROVED,
        });

        return new NextResponse(JSON.stringify({ message: "Request approved" }), {
            status: 200,
            headers: { "Content-Type": "application/json" },
        });
    } catch (error: any) {
        ...
        return new NextResponse(
            JSON.stringify({ message: "Failed to approve join request" }),
